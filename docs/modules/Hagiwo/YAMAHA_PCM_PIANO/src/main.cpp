...
void loop()
{
  ...
  if (Midi)
  { // Initial setting midi data send
    if (initial == 1)
    {
      initial_setting();
      initial = 0;
    ...
    //--------------------analog read POT---------------------------
    if (timer + 50 <= millis())
    {                             // Read at 50 msec intervals to reduce the processing load.
      ...
      { // CH1 setting
        if (mode == 0)
        {
          ...
          { // old_POT for Disables POT reading at the moment of switching.
            cutoff1 = POT1;
          }
    ...
    if (old_toggle2 > 1023 && toggle2 <= 1023)
    { // push sw on
      if (mode == 0)
      {
        if (select_ch == 1)
    ...
    if (gate_delay1 == 1 && gate_delay_timer1 + 5 <= millis())
    {                                                              // delay 5msec for adc latency
    ...
    if (gate_delay2 == 1 && gate_delay_timer2 + 5 <= millis())
    {                                                              // delay 5msec for adc latency
      AD_CH2 = analogRead(9) / 4 * (AD_CH2_calb + 0.0016 * calb2); // 12bit to 10bit
      gate_delay2 = 2;                                             // note on flug
      for (search_qnt2 = 0; search_qnt2 <= 61; search_qnt2++)
      { // quantize
